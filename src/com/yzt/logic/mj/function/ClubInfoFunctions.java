...
//					info.put("maxNums", redisClub.getPersonQuota());//--不用
	...
	public static void interface_500003(WSClient channel, Map<String, Object> readData) throws Exception {
		...
		if (null != clubInfo) {
			...
			info.put("clubUserName", ClubMapper.selectCreateName(StringUtils.parseInt(clubInfo.getCreateId())));
...
//		Integer actNum = ClubMapper.todayPerson(Integer.valueOf(clubId.toString())).size();
		...
		// 根据俱乐部id和时间查询开局数
//		Integer juNum = ClubMapper.todayGames(Integer.valueOf(clubId.toString()));
		//走缓存
...
//		if(clubInfo.getCreateId().equals(userId)){
//			info.put("isCreated", 1);
//		}else{
//			info.put("isCreated", 0);
//		}
		/************************** 未开局的房间数 **********************************/
...
//					Player roomCreater = RedisUtil.getPlayerByUserId(String.valueOf(room.getCreateId()));
					jsobj.put("num", num);// 当前加入房间人数
					roomobj.put("circleNum", room.getCircleNum());
					roomobj.put("scoreType", room.getScoreType());
...
//  			maps.add(roomInfos);
	...
	public synchronized static void interface_500004(WSClient channel, Map<String, Object> readData) {
		...
		room.setCircleNum(circleNum);// 总局数--也可能是总圈数
		room.setLastNum(circleNum);// 剩余局数--也可能是剩余圈数
	...
	public synchronized static void interface_500005(WSClient channel, Map<String, Object> readData) throws Exception {
		...
		info.put("userId", room.getZhuangId());
		info.put("roomSn", roomId);
		info.put("lastNum", room.getLastNum());
		info.put("totalNum", room.getCircleNum());
