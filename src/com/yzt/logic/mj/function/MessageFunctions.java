...
public class MessageFunctions extends TCPGameFunctions {
	...
	public static Map<String, Object> getCurrentUserInfo(Player player, RoomResp room) {
		...
		if(room != null && player.getPlayStatus() == Cnst.PLAYER_STATE_GAME){
			...
			currentUserInfo.put("duiNum", player.getDuiHunNum());
			currentUserInfo.put("chuList", player.getChuList());	
			currentUserInfo.put("lastFaPai",room.getLastFaPai());
			...
			if(room.getNextActionUserId().equals(player.getUserId()))
				currentUserInfo.put("currAction", room.getNextAction());
		}
	...
	public static Map<String, Object> getRoomInfo(RoomResp room) {
		...
		if(room.getState() == Cnst.ROOM_STATE_GAMIING){
			...
			roomInfo.put("lastChuPai", room.getLastChuPai());
			roomInfo.put("lastChuPaiUserId", room.getLastChuPaiUserId());
			if(room.getNextActionUserId().equals(room.getLastFaPaiUserId())&&room.getGuoUserIds().size()==0)
				roomInfo.put("currUserId", room.getLastFaPaiUserId());
			if(room.getNextAction().contains(-1)){
				roomInfo.put("needFaPaiUserId", room.getNextActionUserId());
			}
		}		
	...
	public static void interface_100104(RoomResp room, List<Player> players,Integer interfaceId,Action action) {
		...
		for (Player p : players) {
			...
			if(action.getType() == Cnst.ACTION_TYPE_ANGANG){
				info.put("action", -2);
				info.put("extra",  action.getExtra());				
			}else if(action.getType() == Cnst.ACTION_TYPE_FAPAI){
